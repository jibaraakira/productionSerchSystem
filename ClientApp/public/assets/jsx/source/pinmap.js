...
class Pins extends React.Component {
  ...
  insertPin() {
    return this.state.pinCoordinates.map((coordinate, index) => (
      <div
        class="pin"
...
class Box extends React.Component {
  ...
  componentDidMount() {
    ...
    var element = document.getElementById("map");
    this.resizeClass = new controleSetter(
      element.clientWidth,
  ...
  renderMouseStalker(event) {
    ...
    if (this.isOver(currentPoint)) {
      ...
      stalkerPin.style.transform = `translate(${currentPoint.xPoint - 10}px, ${
        currentPoint.yPoint - 10
      }px)`;
  ...
  isOver(currentPoint) {
    ...
    let xPointIsInner =
      11 <= currentPoint.xPoint && currentPoint.xPoint <= maps.width - 11;
    let yPointIsInner =
      11 <= currentPoint.yPoint && currentPoint.yPoint <= maps.height - 11;

    console.log(
      `10 ${currentPoint.xPoint} ${maps.width},\r\n10 ${currentPoint.yPoint} ${maps.height}`
    );
  ...
  render() {
    ...
    return (
      <div>
        <div class="pin--stalker" onClick={this.addPin.bind(this)}></div>
        <div id="map" onMouseMove={this.moveMouseMove.bind(this)}>
          <Pins init={defa} updateState={this.updateState.bind(this)} />
          {rangePin}
        </div>
        <Buttons updateState={this.updateState.bind(this)} />
      </div>
    );
  }
}
